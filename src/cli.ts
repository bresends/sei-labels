...
export async function cli(options: CLIOptions = {}): Promise<void> {
  ...
  try {
    ...
    const processService = new ProcessService(page, config, selectors, browserService);
    const results: ProcessInfo[] = [];

    for (let i = 0; i < processes.length; i++) {
      const processNumber = processes[i];
      spinner.start(`[${i + 1}/${processes.length}] Processando ${processNumber}...`);
      ...
      if (result.status === 'success') {
        spinner.succeed(chalk.green(`[${i + 1}/${processes.length}] ${processNumber} - Tag adicionada`));
      } else if (result.status === 'skipped') {
        spinner.warn(chalk.yellow(`[${i + 1}/${processes.length}] ${processNumber} - ${result.errorMessage}`));
      } else {
        spinner.fail(chalk.red(`[${i + 1}/${processes.length}] ${processNumber} - ${result.errorMessage}`));
      }
    }
